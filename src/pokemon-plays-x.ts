...
export class PokemonPlaysX {
    ...
    async run(): Promise<void>  {
        this.client.on("message", async (channel: string, tags: ChatUserstate, message: string, self: boolean): Promise<void> => {
    ...
    private isMessageMatching(message: string): RegExpMatchArray | null {
        const commandRegex = new RegExp("^(" + config.commands.join("|") + ")$", "i")
